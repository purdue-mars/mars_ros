...
void ICP::set_mesh_(std::string mesh_name)
{
    ...
    for (int i = 0; i < mesh_pc->points.size(); i++)
    {
        ...
        centroid_x += mesh_pc->points[i].x;
        centroid_y += mesh_pc->points[i].y;
        centroid_z += mesh_pc->points[i].z;
    }

    centroid_x /= mesh_pc->points.size();
    centroid_y /= mesh_pc->points.size();
    centroid_z /= mesh_pc->points.size();

    for (int i = 0; i < mesh_pc->points.size(); i++)
    {
        mesh_pc->points[i].x -= centroid_x;
        mesh_pc->points[i].y -= centroid_y;
        mesh_pc->points[i].z -= centroid_z;
    }
