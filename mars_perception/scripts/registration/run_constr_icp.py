...
import mars_perception.registration.constrained_icp as cicp
...
def main():
    ...
    o3d.global_optimization(pose_graph, o3d.registration.GlobalOptimizationLevenbergMarquardt(),
...
def build_pose_graph(pcds, transforms, dist_coarse, dist_fine):
    ...
    for i in range(1, len(pcds)):
        ...
        #     pose_graph.edges.append(o3d.registration.PoseGraphEdge(i - 2, i, transform, information, uncertain=True))
...
def remove_clouds_outliers(pcds, num_points, radius, ratio=0):
    for i in range(len(pcds)):
        ...
        pcds[i] = o3d.geometry.select_down_sample(pcds[i], ind)

        if ratio > 0:
            cl, ind = o3d.geometry.statistical_outlier_removal(pcds[i], 50, ratio)
            pcds[i] = o3d.geometry.select_down_sample(pcds[i], ind)
...
def estimate_clouds_normals(pcds, radius, max_nn):
    for i in range(len(pcds)):
        o3d.geometry.estimate_normals(
            pcds[i],
            search_param=o3d.registration.KDTreeSearchParamHybrid(radius=radius, max_nn=max_nn))
